...
const float HEAD_MIN = -75.0;
const float HEAD_MAX = 75.0;
const float SHOULDER_PITCH_MIN = 0.0;
const float SHOULDER_PITCH_MAX = 90.0;
...
const float HIP_PITCH_MIN = -15.0;
...
void drawLeg(bool colored, bool left);
...
void drawPenguin(bool colored, bool withLighting) {
    glPushMatrix();
        ...
        GLfloat lightPos[] = {10.0 * sin(lightAngle), 10.0 * cos(lightAngle), 15.0, 0.0};
        GLfloat ambientLight[] = {0.1,0.1,0.1,1.0};

        // only enable lighting if we want it
        if (withLighting) {
            glEnable(GL_LIGHT0);
        ...
        // render pure white if not colored, let lighting handle stuff
        if (!colored) {
            glColor3f(1, 1, 1);
        }
        ...
        drawArm(colored, false);
        drawArm(colored, true);

        // draw right and left legs
        drawLeg(colored, false);
        drawLeg(colored, true);
...
void drawArm(bool colored, bool left) {
    int shoulderPitchDOF = left ? Keyframe::L_SHOULDER_PITCH : Keyframe::R_SHOULDER_PITCH;
    int shoulderYawDOF = left ? Keyframe::L_SHOULDER_YAW : Keyframe::R_SHOULDER_YAW;
    int shoulderRollDOF = left ? Keyframe::L_SHOULDER_ROLL : Keyframe::R_SHOULDER_ROLL;
    int elbowDOF = left ? Keyframe::L_ELBOW : Keyframe::R_ELBOW;
    
    float shoulderPitch = joint_ui_data->getDOF(shoulderPitchDOF);
    float shoulderYaw = joint_ui_data->getDOF(shoulderYawDOF);
    float shoulderRoll = joint_ui_data->getDOF(shoulderRollDOF);
    float elbow = joint_ui_data->getDOF(elbowDOF);
    ...
    glPushMatrix();
        ...
        // move to where shoulder starts
        glTranslatef(0, 2, left ? -1 : 1);
        
        // rotate by shoulder angle
        rotate(shoulderPitch, shoulderYaw, shoulderRoll);
        
        // arm is thin and tall
        glScalef(0.5, 1.5, 0.25);
        
        // make sure cube is drawn so one of the ends connects to the shoulder
        glTranslatef(0, -1, 0);
 
        // render the arm
        drawCube();
        
        // move to where the elbow starts
        glTranslatef(0, -0.8, 0);
        
        // the elbow rotates along the z-axis, because it goes out from the arm
        rotate(0, 0, elbow);
        
        if (colored)
                glColor3f(0.75, 0.75, 0);
        
        // undo scaling and scale for hand size
        glScalef(1, 0.33, 0.5);
...
void drawLeg(bool colored, bool left) {
    ...
    int hipPitchDOF = left ? Keyframe::L_HIP_PITCH : Keyframe::R_HIP_PITCH;
    int hipYawDOF = left ? Keyframe::L_HIP_YAW : Keyframe::R_HIP_YAW;
    int hipRollDOF = left ? Keyframe::L_HIP_ROLL : Keyframe::R_HIP_ROLL;
    int kneeDOF = left ? Keyframe::L_KNEE : Keyframe::R_KNEE;
    
    float hipPitch = joint_ui_data->getDOF(hipPitchDOF);
    float hipYaw = joint_ui_data->getDOF(hipYawDOF);
    float hipRoll = joint_ui_data->getDOF(hipRollDOF);
    float knee = joint_ui_data->getDOF(kneeDOF);
    ...
    glPushMatrix();
        ...
        // move to where leg shoots out of body
        glTranslatef(0, -1, left ? -0.8 : 0.8);
        
        // apply leg rotation
        rotate(hipPitch, hipYaw, hipRoll);
        
        // leg is long and thin
        glScalef(0.4, 2, 0.25);
...
void drawHead(bool colored) {
    glPushMatrix();
        ...
        // to avoid depth-fighting and clipping during rotation, scale the head down slightly so its faces
        // do not overlap with those of the body.
        glScalef(0.8, 1, 0.7);
        
        glPushMatrix();
            rotate(0, joint_ui_data->getDOF(Keyframe::HEAD), 0);
            drawCube();
        glPopMatrix();
        ...
        rotate(0, 0, joint_ui_data->getDOF(Keyframe::BEAK) * 25);
